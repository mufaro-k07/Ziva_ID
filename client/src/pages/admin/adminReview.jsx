...
export function AdminReview() {
  ...
  const handleStatusUpdate = (e) => {
    ...
    try {
    const res = await fetch(`http://localhost:3000/admin/intakes/${record.id}/status`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      credentials: "include", // Passes your BetterAuth cookie/token!
      body: JSON.stringify({
        newStatus: newStatus,
    ...
    setTimeout(() => {
      ...
      setAuditHistory(prev => [newLogEntry, ...prev]);
      setOfficerComment("");
      setIsSubmitting(false);
      alert(`Record ${record.id} successfully updated to ${newStatus.toUpperCase()}! Log stored in audit trail.`);
    }, 600);
  };
      ...
      <main className="admin-main">
        ...
        <header className="admin-topbar">
          ...
          <div style={{ fontWeight: '700', fontSize: '18px', color: '#0F172A' }}>
            Document Inspection & Adjudication: {record.id}
          </div>
